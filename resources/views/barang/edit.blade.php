@extends('layouts.app')
@section('title', $barang->nama_barang.' - Edit Barang | PT Life Mart')
    ...
    <form action="{{ route('updateBarang', $barang->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            @if(Storage::exists('public/images/barang/'.$barang->foto_barang))
                <img src="{{ asset('storage/images/barang/'.$barang->foto_barang) }}" alt="{{ $barang->nama_barang }}" class="w-100 mb-2">
            @else
                <img src="{{ $barang->foto_barang }}" alt="{{ $barang->nama_barang }}" class="w-100 mb-2">
            @endif
